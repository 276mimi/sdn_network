...
def custom_tree_topology():
    ...
    # Initialize the Mininet object with a remote controller and TCLink
    ...
    # Add the core layer switch
    core_switch = net.addSwitch('s1')

    # Add the aggregation layer switches
    agg_switch1 = net.addSwitch('s2')
    agg_switch2 = net.addSwitch('s3')

    # Add the access layer switches
    ...
    # Link the core switch to the aggregation layer switches
    net.addLink(core_switch, agg_switch1, bw=50, delay='1ms', use_htb=True)
    net.addLink(core_switch, agg_switch2, bw=50, delay='1ms', use_htb=True)

    # Link aggregation switches to access layer switches
    net.addLink(agg_switch1, access_switch1, bw=30, delay='2ms', use_htb=True)
    net.addLink(agg_switch1, access_switch2, bw=30, delay='2ms', use_htb=True)
    net.addLink(agg_switch2, access_switch3, bw=30, delay='2ms', use_htb=True)
    net.addLink(agg_switch2, access_switch4, bw=30, delay='2ms', use_htb=True)

    # Link hosts to access layer switches
    ...
    # Start the network
    net.start()
    print("Custom tree topology is up. Use 'pingall' in the CLI to test connectivity.")

    # Test connectivity
    net.pingAll()

    # Enter the Mininet CLI for interactive testing
    CLI(net)
